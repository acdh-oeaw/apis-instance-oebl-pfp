...
class PersonForm(GenericModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields["external_resources"].widget = NewlineSeparatedListWidget(
            attrs={"class": "mb-1"}
        )
        ...
        if {"start_date_written", "end_date_written"} <= set(all_other_fields):
            ...
            startlabel = (
                "Geboren am"
                if self.Meta.model == Person
                else self.fields["start_date_written"].label
            )
            endlabel = (
                "Gestorben am"
                if self.Meta.model == Person
                else self.fields["end_date_written"].label
            )
            row = Row(
                Column(PrependedText("start_date_written", startlabel)),
                Column(PrependedText("end_date_written", endlabel)),
            )
            self.fields["start_date_written"].label = self.fields[
                "end_date_written"
            ].label = False
        ...
        if {"profession", "professioncategory"} <= set(all_other_fields):
            ...
            all_other_fields.insert(
                3, Row(Column("profession"), Column("professioncategory"))
            )
        if {"profession_father", "profession_mother"} <= set(all_other_fields):
            ...
            all_other_fields.insert(
                4, Row(Column("profession_father"), Column("profession_mother"))
            )
