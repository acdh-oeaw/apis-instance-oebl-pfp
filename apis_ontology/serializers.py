...
def add_time_spans(
    g: Graph, ts_node: URIRef, instance: Any, field: str
) -> Graph:
...
class PersonCidocSerializer(E21_PersonCidocSerializer):
    def add_life_event_place(self, g, instance, event_type, event_uri):
        ...
        if event_type == "birth":
            ...
            label_template = "Tod von {}"
            crm_relation = "P98_brought_into_life"
            leg_rel = PersonPlaceLegacyRelation.objects.filter(subj_object_id=instance.pk, legacy_relation_vocab_label="geboren in").first()
        ...
        if event_type == "death":
            ...
            label_template = "Geburt von {}"
            crm_relation = "P100_was_death_of"
            leg_rel = PersonPlaceLegacyRelation.objects.filter(subj_object_id=instance.pk, legacy_relation_vocab_label="gestorben in").first()
        ...
        if event_uri is None:
            event_uri = URIRef(self.attributes_namespace[f"{event_type}_{instance.id}"])
            g.add((event_uri, RDF.type, crm_namespace[crm_type]))
            g.add(
                (
                    event_uri,
                    RDFS.label,
            ...
            g.add((event_uri, crm_namespace[crm_relation], self.instance_uri))
        ...
        g.add((event_uri, crm_namespace.P7_took_place_at, place_uri))
    ...
    def to_representation(self, instance):
        ...
        if instance.start is not None:
            birth_event = URIRef(self.attributes_namespace[f"birth_{instance.id}"])
            birth_time_span = URIRef(self.attributes_namespace[f"birth_time-span_{instance.id}"])
            g.add((birth_event, RDF.type, crm_namespace.E67_Birth))
            g.add((birth_event, RDFS.label, Literal(f"Geburt von {str(instance)}")))
            g.add((birth_event, crm_namespace.P98_brought_into_life, self.instance_uri))
            g.add((birth_event, crm_namespace["P4_has_time-span"], birth_time_span))
        ...
        if instance.end is not None:
            death_event = URIRef(self.attributes_namespace[f"death_{instance.id}"])
            g.add((death_event, RDFS.label, Literal(f"Tod von {str(instance)}")))
            death_time_span = URIRef(self.attributes_namespace[f"death_time-span_{instance.id}"])
            g.add((death_event, RDF.type, crm_namespace.E69_Death))
            g.add((death_event, crm_namespace.P100_was_death_of, self.instance_uri))
            g.add((death_event, crm_namespace["P4_has_time-span"], death_time_span))
